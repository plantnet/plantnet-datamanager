var utils = require('vendor/datamanager/lib/utils'),
    ...
    docLib = require('vendor/datamanager/lib/doc');
...
//deletes all docs in a structure, and possibly deletes views and queries definitions too
...
// like exports.delete_mm_docs but PURGES docs, using a server-side method (because
// data traffic is huge)
exports.purge_mm_docs = function(db, mm_id, onSuccess, onError, includeViewsAndQueries) {
    ...
    db.dm('purge_structure_docs', { 'structId' : mm_id }, null, function(data) {
        $.log('libmm cb', data);
        if (includeViewsAndQueries) {
            $.log('libmm dvq');
            exports.delete_mm_vq(db, mm_id, onSuccess, onError);
        } else {
            $.log('libmm calling back!!');
            onSuccess(data);
        }
    }, function(err) {
        $.log('libmm error');
        onError(err);
    }, null, function(couscous) {
        $.log('libmm oncomplete', couscous);
        onSuccess(couscous);
    });
};
...
//deletes documents for mm / modi or mm / modt
exports.delete_mod_docs = function(db, mm_id, mod, onSuccess, onError, deleteSynonyms) {
    var docs, ids;
    db.view('datamanager/by_mod', {
        key: ['_design/' + mm_id, mod+''],
        cache: JSON.stringify(new Date().getTime()),
        reduce: false,
        success: function(vdocs) {
            ...
            if (ids.length > 0) {
                docLib.delete_with_sons(db, ids, onSuccess, onError, deleteSynonyms);
            } else {
                onError('There is no document in this module.');
            }
        },
        error: function() {
            onError('Error.');
...
// like exports.delete_mod_docs but PURGES docs, using a server-side method (because
// data traffic is huge). Accepts either a modi or a modt
// @TODO should include sons to preserve data consistency!
exports.purge_mod_docs = function(db, mm_id, mod, onSuccess, onError) {

    onError = onError || function() {};

    db.dm('purge_structure_docs', { 'structId' : mm_id, 'mod': mod }, null, onSuccess, onError, null, onSuccess);
};
...
var _get_spatial_func = function(mm_id, modts) {
    var ret = 'function (doc) {' +
        ...
        '         {_id:doc._id, loc:loc, modi:doc.$modi, field:field }); ' +
        '    }' +    
        '}'+
        '   for (var f in doc.$ref) {' +
        '      var loc = doc.$ref[f].geoloc;' +
        '      if (loc && loc.length == 2) {' +
        '          emit({type : "Point", coordinates:[loc[0], loc[1]]}, {_id:doc._id, loc:loc, modi:doc.$modi, field:f}); ' +
        '      }' +
        '    }' +
        '}';  
    return ret;
...
exports.mm_to_view = function (mm, compress, expand_geoloc, export_synonymy, export_label, export_id) {
