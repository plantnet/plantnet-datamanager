...
// data: POST data
var _dm_handler = function(dbname, action, params, data, admin, onSuccess, onError, timeout, onComplete) {
...
// calls server-side dm function in user mode (non-admin)
var _dm_user_handler = function(action, params, data, onSuccess, onError, timeout, onComplete) {
    _dm_handler(this.name, action, params, data, false, onSuccess, onError, timeout, onComplete);
};

// calls server-side dm function in admin mode
exports._dm_admin_handler = function(dbname, action, params, data, onSuccess, onError, timeout, onComplete) {
    _dm_handler(dbname, action, params, data, true, onSuccess, onError, timeout, onComplete);
};

// kind of singleton that retrieves a doc only if it was modified - do we even use this??
...
exports.init_app = function(app, callback) {
    ...
    app.db.dm = _dm_user_handler;
    app.db.dm_admin = exports._dm_admin_handler;
...
// launch that before any page rendering to get sure that previous "stuck" messages
// will be erased when changing page - otherwise lib is kept in cache or whatever
// and the variables don't get reset
exports.clearBusyMsg = function() {
    busy_ids = [];
    busy_msg = {};
}
...
exports.Trigger.prototype.trigger = function(id) {
    ...
    if (exports.is_array(c.obj)) { // multiple triggers!!
        for (var i=0; i < c.obj.length; i++) {
            if (exports.is_array(c.param)) {
                c.obj[i].trigger(c.event, c.param[i]);
            } else {
                c.obj[i].trigger(c.event, c.param);
            }
        }
                ...
                try {
                    ...
                    if (inputValue.split(',').length == 2) {
                        try {
                            ...
                            $.log('could not parse geoloc', inputValue, e);
                        }
                    } else {
                        $.log('could not parse geoloc', inputValue, e);
                    }
...
// returns true if the given file name or url ends with an extension typical to
// images files - handmade - extend the image extensions array as you wish!
exports.hasImageExtension = function(url) {
    var isImage = false,
        extensions = ['png', 'jpg', 'jpeg', 'gif', 'bmp', 'tiff', 'tif'];

    for (var i=0, l=extensions.length; i<l && (! isImage); i++) {
        isImage = (isImage || url.endsWith(extensions[i], true));
    }

    return isImage;
};

// returns the modt given the modi
exports.modtFromModi = function(modi) {
    return modi.substr(modi.lastIndexOf('.') + 1);
};

// returns the parent modi from the modi
exports.guessParentModiFromModi = function(modi) {
    if (! modi || (typeof modi != 'string')) {
        return false;
    }
    var lio = modi.lastIndexOf('.');
    if (lio == 0) { // modi is at root
        return '_root'; // not so standardized...
    } else {
        return modi.substr(0, lio);
    }
}
