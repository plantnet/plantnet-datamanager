...
exports.import_csv = function (db, csv_data, mm, user_ctx, notrim, hasAttachments, col_map, withConflicts, onSuccess, onError, updateStats) {
    ...
    function save_all(db, docs, withConflicts, attchs_err) {
        ...
        db.bulkSave({docs : docs, 'all_or_nothing' : withConflicts}, {
            success : function() {
                updateStructureAndLinkData(db, mm, attchs_err, onSuccess, onError);
            },
            error : function() {
                onError('Error during bulkSave');
            }
        });
    }

    var oneShot = false, // set to true to disable progressive POSTing
        slicesSize = hasAttachments ? 5 : 50; // beware that these are docs slices, and there may be multiple attachments per doc!
...
// called on the end of import: update structure, update Lucene index, match references
// Warning: may fail due to unavailable "update_mm" mutex - tries 3 times at 10 sec. of
// interval then gives up
function updateStructureAndLinkData(db, mm, attchs_err, onSuccess, onError, tries) { // onComplete all slices - update structures and link data
    tries = tries || 3;
    tries --;
    utils.showInfo('Try updating meta data...');
    db.dm('update_mm', {mm : mm._id}, null, function() {
        db.dm('match_ref_mm', {mm : mm._id});
        onSuccess(attchs_err);
    }, function() {
        if (tries > 0) {
            utils.showInfo('Mutex locked! ' + tries + ' attempts remaining - wait 10 sec before retrying');
            setTimeout(function() {
                updateStructureAndLinkData(db, mm, attchs_err, onSuccess, onError, tries);
            }, 10000);
        } else {
            onError('Impossible to update structure. Open and save the structure manually, or ask an administrator to do it');
        }
    });
