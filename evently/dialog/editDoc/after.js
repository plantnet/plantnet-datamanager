function (doc, mm, presets, parent_id, parent_label, parent_modi, synonym_label) {
    ...
    if (firstInput && lastInput) { // does not work with custom multi-enum widget
        ...
        switch (fiType) {
            case 'multi-enum':
                ...
                firstInput = firstInput.parent().find('a.dropdown-toggle').first();
            case 'boolean':
    ...
    function setAutocompleteAndModis() {
        ...
        if (parent_id && (! hasChildren)) {
            ...
            } else { // if parent was deleted but doc still mentions it as its parent (may happen after sync)
                // @TODO all parent modis can be guessed this way. Why est-ce qu'on
                // se traine la variable parent_modi ??
                parentForModis = utilsLib.guessParentModiFromModi(doc.$modi);
                //$.log('guessed parent modi', parentForModis);
            }
        }
        //$.log('parent for modis', parentForModis);
        // adds all acceptable modis to the selector, based on acceptable parents
