function(id, mm, related, synLabels) {
    ...
    for (var subId in subDocs) {
        var subDoc = subDocs[subId],
            ...
            parent_id = utilsLib.get_parent(subDoc),
        ...
        pth = pth.map(function (e) {
            ...
            if(e) {
                if (d === undefined) {
                    mayBeWrongId = true; // I never saw "d" being undefined unless the viewed doc has a wrong id
                    return e + '~~';
                } else {
                    return e + '~~' + d.$modi + d.$label;
                }
            } else {
                return "~~";
            }
        ...
        function add_line(subDoc, outdoc, label, key, type, extra) {
            ...
            if (hasVal) {
                ...
                if (subDoc.$ref && subDoc.$ref[key]) {
                    var tooltip = [];
                    for (var refmodi in subDoc.$ref[key]) {
                        if (refmodi === '_id') {
    ...
    if (mayBeWrongId) {
        utilsLib.showWarning('Some hierarchy information is missing - maybe your doc is in conflict');
    }
    ...
    // if (mm.structure) {
    //     var chk_structure = mmLib.get_sorted_modis(mm);
    //     var layout_order = new Array();
    //     for (var i = 0; i < chk_structure.length; i++) {
    //         if (chk_structure[i].modi) {
    //             var key = chk_structure[i].modi;
    //             layout_order[key] = {'order': i};
    //         }
    //     }
    //     docs.forEach(function(doc) {
    //         if (doc.modi && layout_order[doc.modi]) {
    //             doc.layoutorder = layout_order[doc.modi].order;
    //         }
    //     });
    //     docs.sort(function(a,b) {
