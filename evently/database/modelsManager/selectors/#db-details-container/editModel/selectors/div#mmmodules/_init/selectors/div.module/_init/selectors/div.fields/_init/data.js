function() {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (actualFieldType == 'boolean') {
            ...
            // enum values should be saved as array but in case of a problem...
            if (typeof enumValues === 'string') {
                try {
                    enumValues = JSON.parse(enumValues);
                } catch(e) {
                    $.log('could not parse bad formatted enum values');
                }
            } else {
            ...
            // multi-enum values should be saved as array but in case of a problem...
            if (typeof multiEnumValues === 'string') {
                try {
                    multiEnumValues = JSON.parse(multiEnumValues);
                } catch(e) {
                    $.log('could not parse bad formatted multi-enum values');
                }
            } else {
                // stringify list to keep format
                fields[i].values = JSON.stringify(fields[i].values);
            ...
            var defaultValue = fields[i].default_value,
                isSerial = fields[i].is_serial;
            if (defaultValue === undefined) {
                defaultValue = '';
            }
            fields[i][paramTypeField] = {
                default_value: defaultValue,
                isSerial: isSerial // sert à rien
            };
        } else {
