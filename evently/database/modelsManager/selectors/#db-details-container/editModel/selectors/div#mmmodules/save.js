function() {
    ...
    function parseTypeParam(that, t, field) {
        if (t === 'enum' || t === 'multi-enum') {
            ...
            field.is_serial = ($('input.is-serial', that).attr('checked') == 'checked');
        } else if (t === 'ref') {
    ...
    doc.readonly = ($('#mm_readonly').attr('checked') == 'checked');
    ...
    $('div.module.editor').each(function() {
        ...
        $(this).find('table.fields tbody tr').each(function() {
            var nameField = $('.field-name', this).text(),
                ...
                hidden = $('input.hidefield', this).attr('checked') == 'checked' ? true : false,
