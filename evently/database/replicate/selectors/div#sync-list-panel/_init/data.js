function(replications) {
    ...
    for (var i=0, l=replications.length; i<l; i++) {
        ...
        if (rep.extra && rep.extra.nickname) {
            rep.nickname = rep.extra.nickname;
        }

        // May we restart the replication? Only if it is not currently running.
        rep.canRestart = ((rep._replication_state == 'completed') || (rep._replication_state == 'error'));

        // Stringified information for details panel
        if (rep.extra) {
            rep.sfyWhat = JSON.stringify(rep.extra.what);
        }
    }
