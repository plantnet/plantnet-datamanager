...
exports.generate_taxo_ref = function(app) {
...
exports.generate_individu = function(taxo_ref_mm, app, size) {
    size = size || 100;
    ...
    for (var i = 0; i < size; i++) {
       ...
       var herbierDet = {
        ...
        individu.$sons['determination'] = [individuDet._id, herbierDet._id];
        individu.$sons['observation'] = [individuObs._id];
        individu.$sons['herbier'] = [individuHerb._id];
        individuHerb.$sons['determination'] = [herbierDet._id];
    }
...
exports.generate_data_bota = function(app, onSuccess, onError) {
    var db = app.db,
        docs = [];

    // taxo
    var taxo_ref = exports.generate_taxo_ref(app);
    docs = docs.concat(taxo_ref.docs);
    //docs.push(taxo_ref.mm);

    var bota_data = exports.generate_individu(taxo_ref.mm, app, 100);
    ...
    for (i = 0; i < docs.length; i++) {
        var d = docs[i];
        if (d.$modi === '.individu.determination' && d.taxon === 'Acacia mangium') {
            r.push('##' + d._id.split('##')[1]); // push individu id
        }
    }
    ...
    for (i = 0; i < docs.length; i++) {
        var d = docs[i];
        if (d.$modi === '.individu.determination' && d.taxon.slice(0, 6) === 'Acacia') {
            r.push('##' + d._id.split('##')[1]); // push individu id
        }
    }
    ...
    for (i = 0; i < docs.length; i++) {
        var d = docs[i];
        if (d.$modi === '.individu' && d.date === '1978-02-04') {
            r = r.concat(d.$sons['determination']); // push individu id
        }
    }
    docs.push({ 
        $type: 'query', 
    ...
    db.saveDoc(taxo_ref.mm, {
        success: function() {
            db.saveDoc(bota_data.mm, {
                success: function() {
                    // save new docs
                    db.bulkSave({docs: docs}, {
                        success: function(a,b) {
                            db.dm("match_ref_mm", {mm : taxo_ref.mm._id}, null);
                            db.dm("match_ref_mm", {mm : bota_data.mm._id}, null);
                            onSuccess();
                        },
                        error: onError
                    });
