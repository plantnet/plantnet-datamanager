...
function update_mm(db, mm, cb, forceRebuildLabels) {
        ...
        db.allDocs({
            ...
            if(!err) {
                ...
                r.rows.forEach(function (e) {
                    ...
                    if ((forceRebuildLabels === true) || e.doc.$to_save) { // if not "forceRebuildLabels", recompute only doc with changed tpl
                        docs.push(e.doc);
                    }
                });
...
Lock.protect(q.db, "update_mm", 60 * 60, function (unLockCb) {
    ...
    var mm = null,
        forceRebuildLabels = false;

    if (q.data) {
        mm = q.data.mm; // test if mm has been posted
        forceRebuildLabels = q.data.forceRebuildLabels;
    }
    ...
    if (mm && mm._id) {
        ...
        db.saveDoc(mm, function (err, data) {
            ...
            update_mm(q.db, mm, unLockCb, forceRebuildLabels);
        });
    } else {
        // load mm
...
}, finish, 3); // 3 try in case of lock
