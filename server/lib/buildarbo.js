...
exports.buildarbo = function (viewCols, inputData, related_mod) {
    ...
    function displayArbo(rows, indent) {
        indent = indent || 0;
        for (var i=0; i < rows.length; i++) {
            var row = rows[i];
            if (row.doc) {
                //log(doIndent(indent) + row.doc._id);
                log(doIndent(indent) + ' ' + row.doc.$modi + ' ' + row.doc.$label);
            } else {
                log(doIndent(indent) + 'ERR no doc');
            }
            //log('SONS: ' + JSON.stringify(row.sons));
            if (row.sons) {
                var newSons = [];
                for (var j in row.sons) {
                    //var son = row.sons[j];
                    newSons.push(row.sons[j]);
                    //displayArbo(son, indent + 4);
                }
                displayArbo(newSons, indent + 4);
            }
        }
    }
